...
//   npm run bundle   → dist/dogrulardan-cokgene.html  (easy to email, put on a USB stick, or host)
...
fs.writeFileSync(path.join(root, 'dist', 'dogrulardan-cokgene.html'), html);
console.log('✓ dist/dogrulardan-cokgene.html', (html.length / 1024).toFixed(0) + ' KB');
...
fs.writeFileSync(path.join(root, 'dist', 'dogrulardan-cokgene.artifact.html'), art);
console.log('✓ dist/dogrulardan-cokgene.artifact.html');
