...
async function exportFormat(browser, format, ffmpeg) {
  ...
  const base = `dogrulardan-cokgene_${format}_${CAPS}`;
  ...
  const enc = spawn(ffmpeg, ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', String(FPS), '-c:v', FRAMES === 'jpeg' ? 'mjpeg' : 'png', '-i', '-',
    ...
    '-metadata', 'title=Doğrulardan Çokgene — Çokgenler', mp4], { stdio: ['pipe', 'inherit', 'inherit'] });
...
console.log(`Doğrulardan Çokgene → MP4  (${FPS} fps, captions: ${CAPS}, ${WORKERS} workers)`);
